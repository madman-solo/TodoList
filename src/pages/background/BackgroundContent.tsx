...
import { useState, useEffect } from "react";
import { FaSort, FaSpinner, FaHeart } from "react-icons/fa";
import { useThemeStore } from "../../store";
import {
  backgroundAPI,
  type BackgroundItem,
...
const BackgroundContent = () => {
  // 优化useThemeStore解构，包含myBackgrounds和addToMyBackgrounds
  const { isDarkMode, setBackground, myBackgrounds, addToMyBackgrounds } =
    useThemeStore();
  const [activeCarousel, setActiveCarousel] = useState(0);
  const [sortOrder, setSortOrder] = useState<"asc" | "desc">("asc");
  const [backgrounds, setBackgrounds] = useState<BackgroundItem[]>([]);
  ...
  // 获取背景上下文数据
  const backgroundContext = useBackgroundContext();
  const { background, selectedBackground, setBackgroundData, setAsBackground } =
    backgroundContext as any;
  ...
  // 检查背景是否已在"我的背景"中
  const isInMyBackgrounds = (backgroundId: number) => {
    return myBackgrounds.some((bg: BackgroundItem) => bg.id === backgroundId);
  };

  // 处理"添加到我的背景"功能
  const handleAddToMyBackgrounds = (item: BackgroundItem) => {
    addToMyBackgrounds(item);
  };
      ...
      <div className="content-module vertical-grid">
        {backgrounds.map((item) => (
          ...
          >
            ...
            {/* 添加到我的背景按钮 */}
            <button
              className={`add-to-my-bg-btn ${
                isInMyBackgrounds(item.id) ? "active" : ""
              }`}
              onClick={(e) => {
                e.stopPropagation();
                handleAddToMyBackgrounds(item);
              }}
              style={{
                position: "absolute",
                top: "8px",
                right: "8px",
                background: isInMyBackgrounds(item.id)
                  ? "#e74c3c"
                  : "rgba(255, 255, 255, 0.9)",
                color: isInMyBackgrounds(item.id) ? "white" : "#333",
                border: "none",
                borderRadius: "50%",
                width: "32px",
                height: "32px",
